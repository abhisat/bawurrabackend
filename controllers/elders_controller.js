...
exports.editOrDelete = function(req, res, next){
  if(req.body.option=='edit'){
    Elder.ElderModel.find({'title': req.body.title}, function(err, elder){
      eld = elder;
      res.render('eldersMenuUpdate', {title: 'Elder Update', elder: elder});
  });
  }
  else if(req.body.option=='delete'){
    Elder.ElderModel.remove({'title': req.body.title}, function(err, elder){
      res.redirect('/eldersMenu');
    });
  }
...
exports.editUpdate = function(req, res, next){
  Elder.ElderModel.findOneAndUpdate({'title': eld[0].title}, {title: req.body.title,
  media_1: req.body.media1,
  body: req.body.body,
